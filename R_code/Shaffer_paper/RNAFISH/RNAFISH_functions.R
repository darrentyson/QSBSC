...
numberJackpotsHistogram <- function(plotName, plotDir, data, geneThresholds){
  ...
  ggplot(numJackpotsPerCell,aes(x=numberJackpotGenes))+
    geom_histogram(aes(y=100*(..count..)/sum(..count..)),binwidth=1,fill='goldenrod2',color='black',size=0.2)+
    theme_classic()+
  ...
  ggplot(numJackpotsPerCell,aes(x=numberJackpotGenes))+
    geom_histogram(aes(y=(..count..)),binwidth=1,fill='goldenrod2',color='black',size=0.2)+
    theme_classic()+
